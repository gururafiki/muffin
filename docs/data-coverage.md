...
> reported as facets and are deliberately **NOT** in `market.required_facet` — requiring them
> would report ~71% of the universe permanently broken against data that is correct, which is the
> ETF/`price` miscalibration repeated at four times the scale. A completeness number that is
> structurally unreachable gets ignored within a week, and the real regressions go with it.
> The `sec_filer` dimension is the honest denominator: filter to `yes`.
...
| ~~**Revenue by geography / business line**~~ **SOLVED 2026-08-29, from the filings rather than a vendor** — but see §3.3: the machinery landed before the coverage did — and with it segment **operating income, capex, depreciation, cost of revenue and assets**, none of which any free provider sells | The FMP route stays dead and the warning below stands: `revenue_per_geography` and `revenue-product-segmentation` are **per-symbol gated** — measured 2026-08-29 on the `stable` API (v4 is retired), AAPL/MSFT/GOOGL/NVDA/JPM/XOM/TSM/BABA answer and NEE, PLD, SAP.DE, **ASML (even the US ADR)**, SHEL.L, NESN.SW, 005930.KS, BHP.AX and 7203.T all **402** — and it returns revenue only, never profit. The answer was never a vendor: **every filing's own XBRL instance carries revenue AND operating income per segment**, keyless. See `security-segments` in [data-ingestion.md](data-ingestion.md). Do NOT build this on FMP |
...
| **DART cannot be called from Deno at all — the cache is a correctness dependency** | Measured 2026-09-05. DART serves **TLS 1.2 only** (a forced 1.3 handshake is refused, `tlsv1 alert protocol version`) and offers only the **static-RSA** suite `AES128-GCM-SHA256`, rejecting every ECDHE suite tried. Deno's TLS is rustls, which implements forward-secret key exchange only and has never supported static RSA — so a direct `fetch` fails with `received fatal alert: HandshakeFailure`, on every platform, naming nothing. Every other origin default in `origins.ts` is the real host so http-cache stays removable without an outage; **this is the one where that is false**. The proxy hop is what makes Korea reachable: the function speaks plain HTTP to nginx and OpenResty's OpenSSL does the TLS — verified from inside the container, where `wget https://opendart.fss.or.kr/` answers 200. `http_cache_enabled: false` does not degrade Korea, it disables it |
| **Segment disclosure for JAPAN (EDINET)** | **Measured 2026-08-29 with a live API key, and it fails the same way ESEF does.** EDINET serves a full XBRL package fast (Nintendo's FY2026 annual report: 3.66 MB instance, 6.5 s) — but it carries **8 distinct dimensions and ZERO segment axes**, identical for a large multi-segment filer (Nintendo) and a small manufacturer. The dimensions are equity components, consolidated/non-consolidated, shareholders, directors and share classes. Segment information exists as `jpcrp_cor:NotesSegmentInformationEtcConsolidatedFinancialStatementsTextBlock` — **a text block of escaped HTML**, not dimensioned facts. Extracting it means scraping per-filer HTML tables with Japanese labels, which is a different reliability class from parsing XBRL and is not attempted. The key works and the account is registered, so a future re-test costs nothing |
| ~~**Segment disclosure for KOREA (DART)**~~ **BUILT 2026-09-05** (`kr-filings` + `security-kr-segments`; see [data-ingestion.md](data-ingestion.md) § Korea) | DART is the opposite of ESEF and EDINET: the XBRL package is a full 7.1 MB instance with definition and presentation linkbases, carrying `ifrs-full:ProductsAndServicesAxis` (28), `SegmentConsolidationItemsAxis` (20), `SegmentsAxis` (16) and `GeographicalAreasAxis` (8) — **the same axes the parser already handles for Diageo**. Verified end to end: SK Gas's geography split reconciles exactly to 7,050,068,258,000 KRW. Needed one control-table row (a qualifier pinning `ConsolidatedMember`, because 1,213 of its facts are parent-only `SeparateMember`) and three parser rules, two of which are improvements everywhere. **DART is SLOW from outside Korea** — a 3.6 MB corp-code file timed out twice at 240 s and a 1.2 MB XBRL took 115 s — so a Korean backlog must be paced far more gently than SEC's |
...
| **How far a regulator can reach, measured 2026-08-29** | Segment disclosure comes from a regulator and nowhere else, so this is the ceiling on the feature. **China 2,325 equities / 14 SEC-reachable · Europe 1,438 / 260 · Japan 1,368 / 103 · India 645 / 0 · Taiwan 535 / 2 · Korea 466 / 0 · Hong Kong 383 / 21.** Korea is **built** (2026-09-05: 443 of 466 equities mapped to a DART filer); Europe is the second-largest gap and needs its own answer, since ESEF is measured not viable and the SEC 20-F path already covers 260 of its 1,438 |
| **The segment serving view unioned every period, and the first consumer found it** | Measured 2026-09-02 when the stock page first read `security_segment_current`: it picked the latest period **per member**, so every member spelling a filer had ever used survived at its own newest year. Apple's business-segments axis served **25 members spanning 2008..2025 summing 627.3bn** against a filed FY2025 **416.2bn**; Shell's product split reached 1,201bn against a revenue of 266.9bn. **129 of 216 (security, axis) groups spanned more than one period — 83 of the 104 securities with segments.** The `partition_id` filter was already right; this is a level above it, and every individual number was correct. Fixed in 157 (and 150 for the nested view): choose the period first, then take its members. Reconciling splits went **6 -> 175 of 209** |
| **The remaining 32 are a wrong reconciliation TARGET, not a wrong split** | GE Vernova's three segments sum to a correct **$30.1bn** against a recorded `reconciled_to` of **$487m** — the parser matched the wrong consolidated figure for that filing. The split itself is right. Any consumer must therefore check against the company's own `revenue` metric rather than `reconciled_to`, and treat the two directions differently: over the trunk is a defect, under it is the filer's choice (Novo Nordisk discloses geographies covering 37% of revenue). The stock page's Sankey does exactly that — it declines to draw a split that exceeds revenue, and names the remainder when one falls short |
...
| **Only ONE concept has two companies on it** | `digital-advertising`. `cloud-infrastructure` has one, and `smartphones`, `enterprise-software`, `gaming`, `personal-computers` and six others are seeded and have never matched. This is downstream of the ordering defect above, not of the vocabulary: comparing AWS to Google Cloud needs company #2 and #3, and the queue was spending its budget on Amazon's 1998 10-Q. `segments.comparable_concepts` is now sampled so the number is a line rather than a thing someone notices |
